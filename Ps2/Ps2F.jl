...
@time Vd,Gd,Cd,gn,gkp=Solve_VFI_loop(100)
...
###Checking for SS: Here I am checking for which value of k=k'

for i in 1:length(gkp)
    if gridk[i] == gkp[i]
        println("The vectors have the same value at position $(i)")
        break
    end
end
...
### Now I will use Howard Iteration Algorithm


function Howard(V_old,kp,n,gridk)
    ...
    for i=1:n_k
     V_h[i]=try 
        utility(gridk[i],kp[i],n[i]) + β*V_old[i]
            catch
                -Inf
            end
...
function VFIH(T::Function,H::Function,gridk)
        @unpack z, α, β, η, σ, δ,max_iter,dist_tol,H_tol = p
        n_k    = length(gridk) ; # Number of grid nodes
        V_old  = zeros(n_k)     ; # Initial value, a vector of zeros
        V_dist = 1              ; # Initialize distance
        V_disth=1
        ...
        for iter=1:max_iter
            # Update value function
            V, G_kp, G_c, G_n,G_p = T(V_old,gridk)
            ###Using the updated values to implement Howard's Algorithm
            while V_disth>H_tol
                V1= H(V,G_p,G_n,gridk)
                V_disth=maximum(abs.(V./V1.-1))
                V=V1
            end 
            # Update distance and iterations
            V_dist = maximum(abs.(V./V_old.-1))
            # Update old function
            # Report progress
            if mod(iter,100)==0
                println("   VFI Loop: iter=$iter, dist=",100*V_dist,"%")
...
function Solve_VFI_loopH(nk,gridk)
    # Get Grid
    # Solve VFI
    Vf, G_kpf, G_cf,G_nh,G_kh = VFIH((x,gridk)->Bellman(x,gridk),(y,z,w,gridk)->Howard(y,z,w,gridk),gridk)
    # Return Solution
    return Vf,G_kpf,G_cf,G_nh,G_kh
end

@time V, G_kp, G_c, G_n, G_p = Solve_VFI_loopH(100,gridk)

################ McQueen ############

function VFIMQ(T::Function,gridk)
    @unpack z, α, β, η, σ, δ,max_iter,dist_tol = p
    n_k    = length(gridk) ; # Number of grid nodes
    V_old  = zeros(n_k)     ; # Initial value, a vector of zeros
    V_dist = 1              ; # Initialize distance
    for iter=1:max_iter
        ...
        V_dist = maximum(abs.(V_new./V_old.-1))
        # Update old function
        lb=(β/(1-β))*minimum(V_new-V_old)
        ub=(β/(1-β))*maximum(V_new-V_old)
        distmq=ub-lb
        V_old  = V_new
...
function Solve_VFI_loopMQ(n_k)
    # Get Grid
    gridk=kgrid(nk,p)
    # Solve VFI
    V, G_kp, G_c,G_n,Gp = VFIMQ(x->Bellman(x,gridk),gridk)
    # Return Solution
    return V, G_kp, G_c,G_n,Gp
end


@time Vd,Gd,Cd,gn,gkp=Solve_VFI_loopMQ(100)
